...
class Qwen3VLAgent:
    ...
    def predict(self, instruction: str, obs: Dict) -> List:
        ...
        # Safety check for None screenshots
        if screenshot_bytes is None:
            logger.error("Screenshot is None, returning FAIL action")
            return "Screenshot is None", ["FAIL"]
...
Please generate the next move according to the UI screenshot, instruction and previous actions. You should primarily follow the instruction to decide how many actions are necessary for complete the instruction, and do not generate more actions than necessary, and do not generate less actions that lead to imcomplete of the task.
...
"""
    ...
    def parse_response(
        ...
        def adjust_coordinates(x: float, y: float) -> Tuple[int, int]:
            ...
            pg_w, pg_h = _pg.size()  # logical screen size pyautogui actually uses
            if self.coordinate_type == "absolute":
                # scale from processed image pixels → pyautogui logical pixels
                if processed_width and processed_height:
                    return int(x * pg_w / processed_width), int(y * pg_h / processed_height)
                return int(x), int(y)
            # relative: scale from 0..999 grid → pyautogui logical pixels
            # (do NOT use the physical screenshot size — Retina screens have a 2x
            #  gap between physical pixels and the logical coordinates pyautogui expects)
            return int(x * pg_w / 999), int(y * pg_h / 999)
    ...
    def _call_llm_openai(self, messages, model):
        """Call LLM using OpenAI SDK (compatible with vLLM OpenAI-compatible endpoints)."""
        # Use instance variables if provided, otherwise fall back to environment variables
        base_url = self.base_url or os.environ.get("OPENAI_BASE_URL", "https://dashscope.aliyuncs.com/compatible-mode/v1")
        api_key = self.api_key
        
        logger.info(f"[vLLM] Using base_url: {base_url}")
        client = openai.OpenAI(base_url=base_url, api_key=api_key)

        for attempt in range(1, MAX_RETRY_TIMES + 1):
            logger.info(f"[vLLM] Generating content with model: {model} (attempt {attempt}/{MAX_RETRY_TIMES})")
            try:
                ...
                logger.error(f"[vLLM] Error calling model: {e}")
